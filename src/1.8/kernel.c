...
static void mem_parse_mmap(uint32_t magic, uint32_t mbi)
{
    ...
    extern int initramfs_init(uint32_t magic, uint32_t mbi);
...
void kmain(uint32_t magic, uint32_t mbi)
{
    ...
    if (gui_init(magic, mbi) == 0) {
        ...
        drv_ready("gui-wm", 1);
        {
            extern void splash_draw(int stage);
            splash_draw(0);
        }
    } else {
    ...
    /* initramfs (modulo GRUB) antes do fs: vira a camada ROM */
    initramfs_init(magic, mbi);
    fs_init();
    drv_ready("ramfs", 1);
    {
        extern int fs_rom_is_initramfs(void);
        extern void splash_draw(int stage);
        drv_ready("initramfs", fs_rom_is_initramfs());
        drv_ready("vfs", 1);
    ...
    }
    {
        ...
        usb_init();   /* UHCI/OHCI/EHCI/XHCI por polling (apos pci_scan) */
        audio_init(); /* mixer vol/mute + detect AC97/HDA */
        bright_init(); /* backlight software (padrao 100) */
        if (gui_active) {
            extern void splash_draw(int stage);
            splash_draw(3);
        }
    ...
    if (gui_active) {
        extern void splash_draw(int stage);
        splash_draw(4);
        gui_mark_dirty(); /* WM repinta por cima no proximo poll */
    }
