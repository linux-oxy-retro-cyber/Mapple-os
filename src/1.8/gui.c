...
static void fb_present_rect(int x0, int y0, int x1, int y1)
{
    ...
    extern uint32_t bright_apply(uint32_t c);
    ...
    for (int y = y0; y < y1; y++) {
        ...
        int n = x1 - x0, i = 0;
        if (!dim && (((uintptr_t)s | (uintptr_t)d) & 7) == 0) {
...
/* --- Splash de boot (logo + nome + progresso, direto no fb) --- */
static void fill_rect(int x, int y, int w, int h, uint32_t c);
static void text(int x, int y, const char* s, uint32_t fg, uint32_t bg,
                 int opaque);
static void glyph_scaled(int x, int y, char ch, uint32_t fg, uint32_t bg,
                         int sc, int opaque);

static void splash_pause(void)
{
    /* ~200ms sem PIT (IRQ ainda desligada no boot): loop calibrado */
    for (volatile uint64_t i = 0; i < 30000000ull; i++) {
        __asm__ volatile ("pause");
    }
}

void splash_draw(int stage)
{
    ...
    int cx = (int)fb_w / 2, cy = (int)fb_h / 2 - 60;
    int bw = 320, bx = cx - bw / 2, by = cy + 130;
    int s = stage < 0 ? 0 : (stage > 4 ? 4 : stage);
    if (!gui_active) {
        return;
    }
    fill_rect(0, 0, (int)fb_w, (int)fb_h, pack(8, 8, 20));
    /* MG gigante */
    glyph_scaled(cx - 100, cy - 60, 'M', pack(60, 220, 90), pack(8, 8, 20),
                 6, 1);
    glyph_scaled(cx + 4, cy - 60, 'G', pack(60, 220, 90), pack(8, 8, 20),
                 6, 1);
    text(cx - 104, cy + 4, "mapple 0.13.0  x86_64", C_WHITE, pack(8, 8, 20),
         0);
    text(cx - 104, cy + 18, msgs[s], pack(130, 130, 150), pack(8, 8, 20), 0);
    /* barra de progresso */
    fill_rect(bx - 2, by - 2, bw + 4, 16, pack(60, 60, 80));
    fill_rect(bx, by, bw * (s + 1) / 5, 12, pack(60, 220, 90));
    fb_present_rect(0, 0, (int)fb_w, (int)fb_h);
    if (stage == 0) {
        splash_pause();
    }
}
...
typedef enum {
    ...
    W_FOON, W_PONG, W_BRK, W_TET, W_TORUS, W_TERR, W_TUN
} wtype_t;
...
static const char* win_titles[] = {
    ...
    "Matrix", "Foon", "Pong", "Breakout", "Tetris", "Torus", "Terreno", "Tunel"
};
...
static char expl_msg[72] = { 0 };
static uint32_t expl_bytes = 0;

static void expl_setmsg(const char* s)
{
    size_t k = 0;
    while (s[k] && k < sizeof(expl_msg) - 1) {
        expl_msg[k] = s[k];
        k++;
    }
    expl_msg[k] = '\0';
...
static int expl_cmp(int a, int b)
{
    /* diretorios primeiro, depois alfabetico */
    if (expl_isdir[a] != expl_isdir[b]) {
        return expl_isdir[b] - expl_isdir[a];
    }
    const char* x = expl_names[a];
    const char* y = expl_names[b];
    while (*x && *x == *y) {
        x++;
        y++;
    }
...
static void expl_refresh(void)
{
    ...
    for (i = 0; i < expl_n; i++) {
        expl_bytes += expl_sizes[i];
    }
    /* bolha: dirs primeiro + alfabetico */
    for (i = 0; i < expl_n; i++) {
        for (j = i + 1; j < expl_n; j++) {
            if (expl_cmp(i, j) > 0) {
                char tn[64];
                int ti;
                uint32_t ts;
                size_t k = 0;
                while (expl_names[i][k]) {
                    tn[k] = expl_names[i][k];
                    k++;
                }
                tn[k] = '\0';
                k = 0;
                while (expl_names[j][k]) {
                    expl_names[i][k] = expl_names[j][k];
                    k++;
                }
                expl_names[i][k] = '\0';
                k = 0;
                ...
                expl_names[j][k] = '\0';
                ti = expl_isdir[i];
                expl_isdir[i] = expl_isdir[j];
                expl_isdir[j] = ti;
                ts = expl_sizes[i];
                expl_sizes[i] = expl_sizes[j];
                expl_sizes[j] = ts;
            }
        }
    }
...
/* caminho completo do item i em out */
static void expl_fullpath(int i, char* out, size_t cap)
{
    size_t n = 0;
    size_t pn = strlen(expl_path);
    for (size_t k = 0; k < pn && n < cap - 1; k++) {
        out[n++] = expl_path[k];
    }
    if (n == 0 || out[n - 1] != '/') {
        if (n + 1 < cap) {
    ...
    for (size_t k = 0; expl_names[i][k] && n < cap - 1; k++) {
        out[n++] = expl_names[i][k];
    }
    out[n] = '\0';
}

/* cria arquivo/pasta com nome livre (novo-N.txt / pasta-N) */
static void expl_new(int is_dir)
{
    char fp[192];
    int n;
    for (n = 1; n < 100; n++) {
        ...
        {
            const char* base = is_dir ? "pasta-" : "novo-";
            while (*base && k + 12 < sizeof(fp)) {
                fp[k++] = *base++;
            }
        }
        if (v == 0) {
        ...
        for (t = nk - 1; t >= 0 && k + 6 < sizeof(fp); t--) {
            fp[k++] = rev[t];
        }
        if (!is_dir && k + 5 < sizeof(fp)) {
            fp[k++] = '.';
            fp[k++] = 't';
            fp[k++] = 'x';
            fp[k++] = 't';
        ...
        fp[k] = '\0';
        {
            ...
            if (is_dir ? (fs_mkdir(fp) == 0) : (fs_touch(fp) == 0)) {
                expl_setmsg(is_dir ? "pasta criada" : "arquivo criado");
            } else {
                expl_setmsg("falhou (overlay cheio?)");
            }
            expl_refresh();
            gui_mark_dirty();
            return;
    ...
    expl_setmsg("sem nome livre");
}

static void expl_delete_sel(void)
{
    char fp[192];
    if (expl_sel < 0 || expl_sel >= expl_n) {
        expl_setmsg("nada selecionado");
        return;
    }
    expl_fullpath(expl_sel, fp, sizeof(fp));
    if (expl_isdir[expl_sel]) {
        int nf = 0, nd = 0;
        int r = fs_rm_recursive(fp, &nf, &nd);
        if (r == -2) {
            expl_setmsg("somente leitura (ROM)");
        } else if (r != 0) {
            expl_setmsg("nao apagou (montado?)");
        } else {
            expl_setmsg("pasta apagada (-r)");
        }
    } else {
        int r = fs_rm(fp);
        if (r == -2) {
            ...
            expl_setmsg("nao apagou (virtual?)");
        } else {
            expl_setmsg("arquivo apagado");
        }
    }
    expl_refresh();
    gui_mark_dirty();
...
static uint32_t g_seed = 246813579;
static uint32_t g_rand(void)
{
    g_seed = g_seed * 1103515245u + 12345u;
    return (g_seed >> 16) & 0x7FFFu;
}

/* tecla sem bloquear (1 por iteracao); 1 se leu */
static int game_key(char* out)
{
    int r = 0;
    __asm__ volatile ("cli");
    if (appq_h != appq_t) {
...
static void game_quit(wtype_t t)
{
    int i = win_find(t);
    win_close(i);
    {
        int k = win_find(W_TERM);
        if (k >= 0) {
            win_focus(k);
        }
    }
...
static void game_touch(wtype_t t)
{
    int i = win_find(t);
    if (i >= 0) {
        gui_invalidate(wins[i].x, wins[i].y,
                       wins[i].x + wins[i].w, wins[i].y + wins[i].h);
    }
}

/* inteiro decimal na tela */
static void gnum(int x, int y, int v, uint32_t fg, uint32_t bg)
{
    char b[12];
    int n = 0, neg = 0;
    char tmp[12];
...
static int game_modal(wtype_t t)
{
    int i = win_find(t);
    return i >= 0 && !wins[i].minimized && focused == i;
}

int gui_foon_modal(void)
{
    return game_modal(W_FOON);
}
int gui_pong_modal(void)
{
    return game_modal(W_PONG);
}
int gui_brk_modal(void)
{
    return game_modal(W_BRK);
}
int gui_tet_modal(void)
{
    return game_modal(W_TET);
}
int gui_game_modal(void)
{
    return gui_foon_modal() || gui_pong_modal() || gui_brk_modal() || gui_tet_modal();
}

/* --- Foon: raycaster estilo Doom (inteiro, Q10) --- */
#define FOON_MW 16
#define FOON_MH 12
static const char foon_map[FOON_MH][FOON_MW + 1] = {
    "################",
    "#......#.......#",
    "#..##..#..###..#",
    "#..##.....#....#",
...
/* Q10: 1024 = 1 celula */
static int32_t foon_px, foon_py;
static int foon_ang; /* 0-255 (tabela fsin/fcos) */
static int foon_hp, foon_score, foon_dead, foon_win;
#define FOON_NEN 4
static int32_t foon_ex[FOON_NEN], foon_ey[FOON_NEN];
static int foon_ehp[FOON_NEN];

static void foon_enemy_spawn(int i)
{
    int x, y, t = 0;
    do {
        x = 1 + (int)(g_rand() % (FOON_MW - 2));
        y = 1 + (int)(g_rand() % (FOON_MH - 2));
        t++;
    } while (foon_map[y][x] == '#' && t < 60);
    foon_ex[i] = x * 1024 + 512;
    foon_ey[i] = y * 1024 + 512;
    foon_ehp[i] = 3;
}

static void foon_reset(void)
{
    int i;
    foon_px = 2 * 1024 + 512;
    foon_py = 1 * 1024 + 512;
    ...
    foon_dead = 0;
    foon_win = 0;
    g_seed = (uint32_t)pit_get_ticks() * 1103515245u + 12345u;
    for (i = 0; i < FOON_NEN; i++) {
        foon_enemy_spawn(i);
    }
}

static int foon_solid(int32_t x, int32_t y)
{
    int cx = (int)(x / 1024), cy = (int)(y / 1024);
    if (cx < 0 || cy < 0 || cx >= FOON_MW || cy >= FOON_MH) {
        return 1;
    }
    return foon_map[cy][cx] == '#';
}

/* anda (dx,dy em Q10); desliza nas paredes eixo a eixo */
static void foon_move(int32_t dx, int32_t dy)
{
    int32_t nx = foon_px + dx, ny = foon_py + dy;
    if (!foon_solid(nx, foon_py)) {
        foon_px = nx;
    }
    if (!foon_solid(foon_px, ny)) {
        foon_py = ny;
    }
}

/* tiro hitscan no raio central: demonio mais proximo na mira sofre 1 */
static void foon_shoot(int32_t walld)
{
    int i, best = -1;
    int32_t bd = walld;
    int dx = fcos(foon_ang), dy = fsin(foon_ang);
    audio_beep(180, 60);
    for (i = 0; i < FOON_NEN; i++) {
        int32_t ex, ey, along, across, ad;
        if (foon_ehp[i] <= 0) {
            continue;
        }
        ex = foon_ex[i] - foon_px;
        ey = foon_ey[i] - foon_py;
        along = (ex * dx + ey * dy) / 1024; /* projecao no raio (Q10) */
        if (along < 300 || along > bd) {
            continue;
        }
        across = (ex * dy - ey * dx) / 1024;
        ad = across < 0 ? -across : across;
        if (ad > 320) {
            continue; /* fora da mira (~17 graus) */
        }
        if (along < bd) {
            bd = along;
            best = i;
    ...
    if (best >= 0) {
        ...
        if (foon_ehp[best] <= 0) {
            ...
            if (foon_score >= 400) {
                ...
                foon_enemy_spawn(best);
            }
        }
    }
}

/* demonios andam ate o jogador; encostou = dano */
static void foon_step(void)
{
    int i;
    if (foon_dead || foon_win) {
        return;
    }
    for (i = 0; i < FOON_NEN; i++) {
        int32_t dx, dy, adx, ady, dist;
        if (foon_ehp[i] <= 0) {
            continue;
        }
        dx = foon_px - foon_ex[i];
        dy = foon_py - foon_ey[i];
        adx = dx < 0 ? -dx : dx;
        ady = dy < 0 ? -dy : dy;
        dist = adx > ady ? adx + ady / 2 : ady + adx / 2;
        if (dist < 700) {
            foon_hp -= 8;
            audio_beep(120, 150);
            foon_enemy_spawn(i);
        ...
        /* passo de 0.06 celula em direcao ao jogador */
        {
            int32_t sx = dx * 60 / (dist + 1);
            int32_t sy = dy * 60 / (dist + 1);
            int32_t nx = foon_ex[i] + sx;
            if (!foon_solid(nx, foon_ey[i])) {
                foon_ex[i] = nx;
            }
            {
                int32_t ny = foon_ey[i] + sy;
                if (!foon_solid(foon_ex[i], ny)) {
                    foon_ey[i] = ny;
                }
            }
        }
...
void foon_run_loop(void)
{
    uint64_t next = pit_get_ticks() + 8;
    for (;;) {
        char c = 0;
        int has = 0;
        if (!gui_foon_modal()) {
        ...
        if (has) {
            ...
            dx = fcos(foon_ang);
            dy = fsin(foon_ang);
            if (c == 'w' || c == 'W' || c == (char)0x81) {
                foon_move(dx * 160 / 1024, dy * 160 / 1024);
            } else if (c == 's' || c == 'S' || c == (char)0x82) {
                foon_move(-dx * 160 / 1024, -dy * 160 / 1024);
            } else if (c == 'a' || c == 'A' || c == (char)0x83) {
                foon_ang = (foon_ang - 6) & 255;
            } else if (c == 'd' || c == 'D' || c == (char)0x84) {
                foon_ang = (foon_ang + 6) & 255;
            } else if (c == ' ') {
                /* parede do raio central p/ limitar o tiro */
                int32_t walld = 8 * 1024;
                int32_t rx = foon_px, ry = foon_py;
                int k;
                for (k = 0; k < 64; k++) {
                    int32_t ddx, ddy, adx, ady;
                    rx += dx / 8;
                    ry += dy / 8;
                    if (foon_solid(rx, ry)) {
                        ddx = rx - foon_px;
                        ...
                        adx = ddx < 0 ? -ddx : ddx;
                        ady = ddy < 0 ? -ddy : ddy;
                        walld = adx > ady ? adx + ady / 2 : ady + adx / 2;
                        break;
                    }
                }
                foon_shoot(walld);
...
/* render: 1 raio a cada 4px + billboards + HUD + minimapa */
static void draw_foon_client(int cx, int cy, int cw, int ch)
{
    int ncol = cw / 4;
    int i;
    int dx0 = fcos(foon_ang), dy0 = fsin(foon_ang);
    /* plano da camera: perpendicular * 0.64 */
    int planx = -dy0 * 655 / 1024, plany = dx0 * 655 / 1024;
    if (ncol > 160) {
        ncol = 160;
    }
    sin_build();
    fill_rect(cx, cy, cw, ch / 2, pack(8, 8, 24)); /* teto */
    fill_rect(cx, cy + ch / 2, cw, ch - ch / 2, pack(48, 48, 48)); /* chao */
    for (i = 0; i < ncol; i++) {
        /* cameraX em Q10 (-1024..1024) */
        int32_t cam = (int32_t)((i * 2048 / (ncol - 1)) - 1024);
        int32_t rdx = dx0 + planx * cam / 1024;
        int32_t rdy = dy0 + plany * cam / 1024;
        int mapx = (int)(foon_px / 1024), mapy = (int)(foon_py / 1024);
        int32_t ddx, ddy, sdx, sdy;
        int stx, sty, side = 0, hit = 0, k;
        int32_t perp;
        int lh, y0, y1;
        uint32_t col;
        int bri;
        ...
        if (rdx < 0) {
            stx = -1;
            sdx = ((foon_px - mapx * 1024) * ddx) / 1024;
        } else {
            stx = 1;
            sdx = (((mapx + 1) * 1024 - foon_px) * ddx) / 1024;
        }
        if (rdy < 0) {
            sty = -1;
            sdy = ((foon_py - mapy * 1024) * ddy) / 1024;
        } else {
            sty = 1;
            sdy = (((mapy + 1) * 1024 - foon_py) * ddy) / 1024;
        }
        for (k = 0; k < 32 && !hit; k++) {
            if (sdx < sdy) {
                sdx += ddx;
            ...
            if (mapx < 0 || mapy < 0 || mapx >= FOON_MW || mapy >= FOON_MH) {
                hit = 1;
                break;
            }
            if (foon_map[mapy][mapx] == '#') {
                hit = 1;
            }
        }
        perp = (side == 0) ? (sdx - ddx) : (sdy - ddy);
        /* DDA ja devolve distancia perpendicular (sem fish-eye) */
        if (perp < 200) {
            perp = 200;
        }
        lh = (int)((int64_t)ch * 900 / perp);
        if (lh > ch * 2) {
            lh = ch * 2;
        }
        y0 = cy + (ch - lh) / 2;
        ...
        col = pack((uint8_t)(200 * bri / 255), (uint8_t)(70 * bri / 255),
                   (uint8_t)(50 * bri / 255));
        fill_rect(cx + i * 4, y0, 4, y1 - y0, col);
    }
    /* demonios (billboards, longe primeiro) */
    {
        int ord[FOON_NEN], dd[FOON_NEN], a, b, e;
        for (e = 0; e < FOON_NEN; e++) {
            int32_t rx = foon_ex[e] - foon_px, ry = foon_ey[e] - foon_py;
            int32_t ax = rx < 0 ? -rx : rx, ay = ry < 0 ? -ry : ry;
            dd[e] = (int)(ax > ay ? ax + ay / 2 : ay + ax / 2);
            ord[e] = e;
        }
        for (a = 0; a < FOON_NEN; a++) {
            for (b = a + 1; b < FOON_NEN; b++) {
                if (dd[ord[a]] < dd[ord[b]]) {
                    int t = ord[a];
                    ord[a] = ord[b];
                    ord[b] = t;
                }
        ...
        for (a = 0; a < FOON_NEN; a++) {
            e = ord[a];
            if (foon_ehp[e] <= 0 || dd[e] < 200) {
                continue;
            }
            {
                int32_t rx = foon_ex[e] - foon_px, ry = foon_ey[e] - foon_py;
                /* transforma p/ espaco da camera */
                int32_t tx = (rx * dx0 + ry * dy0) / 1024; /* profundidade */
                int32_t ty = (rx * plany - ry * planx) / 655; /* lateral */
                if (tx > 300) {
                    int sx = cx + cw / 2 + (int)(ty * (cw / 2) / (tx + 1));
                    int sz = (int)((int64_t)ch * 700 / tx);
                    int se;
                    if (sz > ch) {
                        sz = ch;
                    }
                    se = cy + ch / 2 + sz / 4 - sz;
                    /* corpo */
                    fill_rect(sx - sz / 4, se, sz / 2, sz / 2,
                              pack(200, 30, 30));
                    /* olhos */
                    fill_rect(sx - sz / 8, se + sz / 8, sz / 8, sz / 8,
                              pack(255, 220, 0));
                    fill_rect(sx + 0, se + sz / 8, sz / 8, sz / 8,
                              pack(255, 220, 0));
                    /* barra de vida */
                    fill_rect(sx - sz / 4, se - 6, (sz / 2) * foon_ehp[e] / 3,
                              4, pack(0, 220, 0));
                }
            }
        }
    ...
    /* mira */
    fill_rect(cx + cw / 2 - 6, cy + ch / 2, 13, 2, pack(0, 255, 0));
    fill_rect(cx + cw / 2, cy + ch / 2 - 6, 2, 13, pack(0, 255, 0));
    /* HUD */
    text(cx + 8, cy + 6, "HP", C_WHITE, C_BLACK, 0);
    gnum(cx + 32, cy + 6, foon_hp, foon_hp > 30 ? 0x00FF00 : 0xFF0000, C_BLACK);
    text(cx + 90, cy + 6, "PTS", C_WHITE, C_BLACK, 0);
    gnum(cx + 122, cy + 6, foon_score, 0xFFFF00, C_BLACK);
    text(cx + 8, cy + ch - 14, "WASD/setas move Esp atira q sai", 0xAAAAAA,
         C_BLACK, 0);
    if (foon_dead) {
        text(cx + cw / 2 - 100, cy + ch / 2 - 30, "VOCE MORREU (enter)",
             0xFF0000, C_BLACK, 0);
    }
    if (foon_win) {
        text(cx + cw / 2 - 96, cy + ch / 2 - 30, "FASE LIMPA! (enter)",
             0x00FF00, C_BLACK, 0);
    }
    /* minimapa */
    {
        int mx = cx + cw - FOON_MW * 5 - 8, my = cy + 6, x, y;
        fill_rect(mx - 2, my - 2, FOON_MW * 5 + 4, FOON_MH * 5 + 4,
                  pack(0, 0, 0));
        for (y = 0; y < FOON_MH; y++) {
            for (x = 0; x < FOON_MW; x++) {
                if (foon_map[y][x] == '#') {
                    fill_rect(mx + x * 5, my + y * 5, 5, 5,
                              pack(90, 90, 120));
                }
            }
        }
        for (x = 0; x < FOON_NEN; x++) {
            if (foon_ehp[x] > 0) {
                fill_rect(mx + (int)(foon_ex[x] * 5 / 1024) - 1,
                          my + (int)(foon_ey[x] * 5 / 1024) - 1, 3, 3,
                          pack(255, 0, 0));
            }
        }
        fill_rect(mx + (int)(foon_px * 5 / 1024) - 1,
                  my + (int)(foon_py * 5 / 1024) - 1, 3, 3, pack(0, 255, 0));
    }
}

/* --- Pong: jogador (esq, W/S) x CPU (dir). 5 pontos vencem. --- */
static int po_py, po_cy, po_bx, po_by, po_bdx, po_bdy;
static int po_ps, po_cs, po_over;
static int po_w, po_h;
...
static void pong_step(void)
{
    ...
    /* raquete do jogador */
    if (po_bdx < 0 && po_bx < 22 && po_bx > 8 &&
        po_by > po_py - 26 && po_by < po_py + 26) {
        po_bx = 22;
        po_bdx = -po_bdx;
        po_bdy += (po_by - po_py) / 9;
        if (po_bdy > 4) {
            po_bdy = 4;
        }
        if (po_bdy < -4) {
    ...
    /* raquete da CPU */
    if (po_bdx > 0 && po_bx > po_w - 22 && po_bx < po_w - 8 &&
        po_by > po_cy - 26 && po_by < po_cy + 26) {
        po_bx = po_w - 22;
        po_bdx = -po_bdx;
        audio_beep(500, 30);
    }
    /* CPU segue a bola (limitada) */
    if (po_cy < po_by - 4) {
        po_cy += 3;
    } else if (po_cy > po_by + 4) {
        po_cy -= 3;
    }
    if (po_bx < 0) {
        po_cs++;
...
void pong_run_loop(void)
{
    uint64_t next = pit_get_ticks() + 4;
    for (;;) {
        char c = 0;
        if (!gui_pong_modal()) {
            return;
...
static void draw_pong_client(int cx, int cy, int cw, int ch)
{
    int ox = cx + (cw - po_w) / 2, oy = cy + (ch - po_h) / 2, y;
    fill_rect(cx, cy, cw, ch, pack(8, 10, 18));
    fill_rect(ox, oy, po_w, po_h, pack(0, 0, 0));
    for (y = 0; y < po_h; y += 12) {
        fill_rect(ox + po_w / 2 - 1, oy + y, 2, 6, pack(60, 60, 70));
    }
    fill_rect(ox + 10, oy + po_py - 24, 8, 48, pack(60, 220, 80));
    fill_rect(ox + po_w - 18, oy + po_cy - 24, 8, 48, pack(220, 80, 60));
    fill_rect(ox + po_bx - 4, oy + po_by - 4, 8, 8, C_WHITE);
    gnum(ox + po_w / 2 - 60, oy + 8, po_ps, 0x00FF00, C_BLACK);
    gnum(ox + po_w / 2 + 44, oy + 8, po_cs, 0xFF5555, C_BLACK);
    text(cx + 8, cy + ch - 14, "W/S ou setas (5 pts, q sai)", 0xAAAAAA,
         pack(8, 10, 18), 0);
    if (po_over) {
        text(ox + po_w / 2 - 110, oy + po_h / 2 - 8,
             po_ps >= 5 ? "VOCE VENCEU! (enter)" : "CPU VENCEU (enter)",
             0xFFFF00, C_BLACK, 0);
    }
}

/* --- Breakout: raquete + tijolos. 3 vidas. --- */
#define BR_COLS 10
#define BR_ROWS 6
static int br_px, br_bx, br_by, br_bdx, br_bdy;
static int br_grid[BR_ROWS][BR_COLS];
static int br_lives, br_score, br_over, br_win;
static int br_w, br_h;
...
static void brk_step(void)
{
    ...
    /* raquete */
    if (br_bdy > 0 && br_by > br_h - 24 && br_by < br_h - 8 &&
        br_bx > br_px - 34 && br_bx < br_px + 34) {
        br_by = br_h - 24;
        br_bdy = -br_bdy;
        br_bdx = (br_bx - br_px) / 8;
        if (br_bdx == 0) {
            br_bdx = (g_rand() % 2) ? 1 : -1;
        }
        audio_beep(600, 30);
    ...
    for (r = 0; r < BR_ROWS; r++) {
        for (c = 0; c < BR_COLS; c++) {
            ...
            left++;
            {
                int bx = 10 + c * 38, by = 30 + r * 16;
                if (br_bx > bx - 3 && br_bx < bx + 38 + 3 &&
                    br_by > by - 3 && br_by < by + 13 + 3) {
                    br_grid[r][c] = 0;
                    br_bdy = -br_bdy;
                    br_score += 10;
                    audio_beep(800, 30);
...
static void draw_brk_client(int cx, int cy, int cw, int ch)
{
    int ox = cx + (cw - br_w) / 2, oy = cy + (ch - br_h) / 2;
    int r, c;
    static const uint32_t brow[BR_ROWS] = {
        0xFF4040, 0xFF9040, 0xFFE040, 0x40FF40, 0x40C0FF, 0xC080FF
    };
    fill_rect(cx, cy, cw, ch, pack(10, 8, 20));
    fill_rect(ox, oy, br_w, br_h, pack(0, 0, 0));
    for (r = 0; r < BR_ROWS; r++) {
        for (c = 0; c < BR_COLS; c++) {
            if (br_grid[r][c]) {
                fill_rect(ox + 10 + c * 38, oy + 30 + r * 16, 36, 12,
                          brow[r]);
            }
        }
    }
    fill_rect(ox + br_px - 32, oy + br_h - 16, 64, 8, pack(80, 160, 255));
    fill_rect(ox + br_bx - 3, oy + br_by - 3, 7, 7, C_WHITE);
    text(ox + 8, oy + 6, "PTS", 0xAAAAAA, C_BLACK, 0);
    gnum(ox + 40, oy + 6, br_score, 0xFFFF00, C_BLACK);
    text(ox + br_w - 90, oy + 6, "VIDAS", 0xAAAAAA, C_BLACK, 0);
    gnum(ox + br_w - 34, oy + 6, br_lives, 0x00FF00, C_BLACK);
    text(cx + 8, cy + ch - 14, "A/D ou setas (q sai)", 0xAAAAAA,
         pack(10, 8, 20), 0);
    if (br_over) {
        text(ox + br_w / 2 - 96, oy + br_h / 2 - 8, "FIM DE JOGO (enter)",
             0xFF4040, C_BLACK, 0);
    }
    if (br_win) {
        text(ox + br_w / 2 - 88, oy + br_h / 2 - 8, "VOCE VENCEU! (enter)",
             0x00FF00, C_BLACK, 0);
    }
}

/* --- Tetris: 10x20, 7 pecas, niveis. --- */
#define TT_W 10
#define TT_H 20
static uint8_t tt_grid[TT_H][TT_W];
static int tt_px, tt_py, tt_piece, tt_rot, tt_next;
static int tt_score, tt_lines, tt_level, tt_over;
static const uint16_t tt_shapes[7] = {
    0x0F00, /* I */
    0x6600, /* O */
    0x4E00, /* T */
    0x6C00, /* S */
...
static const uint32_t tt_cols[7] = {
    0x40E0FF, 0xFFE040, 0xC080FF, 0x40FF40, 0xFF4040, 0x4060FF, 0xFF9040
};

/* celula (r,c 0-3) da peca p com rotacao rot (0-3, horaria) */
static int tt_cell(int p, int rot, int r, int c)
{
    int rr = r, cc = c, k;
    for (k = 0; k < rot; k++) {
        int nr = cc, nc = 3 - rr;
    ...
    return (tt_shapes[p] >> (15 - (rr * 4 + cc))) & 1;
}

static int tt_collide(int p, int rot, int px, int py)
{
    int r, c;
    for (r = 0; r < 4; r++) {
        for (c = 0; c < 4; c++) {
            if (tt_cell(p, rot, r, c)) {
                int x = px + c, y = py + r;
                if (x < 0 || x >= TT_W || y >= TT_H) {
                    return 1;
                }
                if (y >= 0 && tt_grid[y][x]) {
                    return 1;
...
{
    tt_piece = tt_next;
    tt_next = (int)(g_rand() % 7);
    tt_rot = 0;
    tt_px = 3;
    tt_py = 0;
    if (tt_collide(tt_piece, 0, tt_px, tt_py)) {
        tt_over = 1;
    }
}
...
static void tt_lock(void)
{
    ...
    for (r = 0; r < 4; r++) {
        for (c = 0; c < 4; c++) {
            if (tt_cell(tt_piece, tt_rot, r, c)) {
                int x = tt_px + c, y = tt_py + r;
                if (y >= 0 && y < TT_H && x >= 0 && x < TT_W) {
                    tt_grid[y][x] = (uint8_t)(tt_piece + 1);
                }
            }
        }
    }
    ...
    if (cleared) {
        ...
        tt_lines += cleared;
        tt_score += pts[cleared > 4 ? 4 : cleared] * tt_level;
        tt_level = tt_lines / 10 + 1;
        if (tt_level > 10) {
            tt_level = 10;
        }
        audio_beep(500 + tt_level * 60, 80);
    }
    tt_spawn();
}
...
static void tt_gravity(void)
{
    ...
    if (!tt_collide(tt_piece, tt_rot, tt_px, tt_py + 1)) {
        tt_py++;
    } else {
        tt_lock();
    }
...
void tet_run_loop(void)
{
    uint64_t next = pit_get_ticks() + 20;
    for (;;) {
        char c = 0;
        if (!gui_tet_modal()) {
            return;
        ...
        if (game_key(&c)) {
            ...
            if (c == '\n' && tt_over) {
                ...
                if (c == (char)0x83 || c == 'a' || c == 'A') {
                    if (!tt_collide(tt_piece, tt_rot, tt_px - 1, tt_py)) {
                        tt_px--;
                    }
                } else if (c == (char)0x84 || c == 'd' || c == 'D') {
                    if (!tt_collide(tt_piece, tt_rot, tt_px + 1, tt_py)) {
                        tt_px++;
                    }
                } else if (c == (char)0x82 || c == 's' || c == 'S') {
                    if (!tt_collide(tt_piece, tt_rot, tt_px, tt_py + 1)) {
                        tt_py++;
                        tt_score++;
                    } else {
                ...
                } else if (c == (char)0x81 || c == 'w' || c == 'W' ||
                           c == 'x' || c == 'X') {
                    int nr = (tt_rot + 1) & 3;
                    if (!tt_collide(tt_piece, nr, tt_px, tt_py)) {
                        tt_rot = nr;
                    }
                } else if (c == ' ') {
                    while (!tt_collide(tt_piece, tt_rot, tt_px, tt_py + 1)) {
                        tt_py++;
                        tt_score += 2;
                    }
                    tt_lock();
        ...
        {
            int speed = 32 - tt_level * 2;
            if (speed < 6) {
                speed = 6;
            }
            if (pit_get_ticks() >= next) {
                next = pit_get_ticks() + (uint64_t)speed;
                tt_gravity();
                game_touch(W_TET);
            }
        }
...
static void draw_tet_client(int cx, int cy, int cw, int ch)
{
    int cell = 13, ox, oy, r, c;
    if (cell * TT_H > ch - 40) {
        cell = (ch - 40) / TT_H;
    }
    ox = cx + (cw - (TT_W * cell + 110)) / 2;
    oy = cy + (ch - TT_H * cell) / 2;
    fill_rect(cx, cy, cw, ch, pack(12, 10, 24));
    fill_rect(ox - 2, oy - 2, TT_W * cell + 4, TT_H * cell + 4,
              pack(60, 60, 80));
    fill_rect(ox, oy, TT_W * cell, TT_H * cell, pack(0, 0, 0));
    for (r = 0; r < TT_H; r++) {
        for (c = 0; c < TT_W; c++) {
            if (tt_grid[r][c]) {
                fill_rect(ox + c * cell + 1, oy + r * cell + 1, cell - 2,
                          cell - 2, tt_cols[tt_grid[r][c] - 1]);
            }
        }
    }
    if (!tt_over) {
        for (r = 0; r < 4; r++) {
            for (c = 0; c < 4; c++) {
                if (tt_cell(tt_piece, tt_rot, r, c)) {
                    int x = tt_px + c, y = tt_py + r;
                    if (y >= 0) {
                        fill_rect(ox + x * cell + 1, oy + y * cell + 1,
                                  cell - 2, cell - 2, tt_cols[tt_piece]);
                    }
                }
            }
        }
    ...
    /* painel */
    {
        int px = ox + TT_W * cell + 14;
        text(px, oy, "PROX", 0xAAAAAA, pack(12, 10, 24), 0);
        for (r = 0; r < 4; r++) {
            for (c = 0; c < 4; c++) {
                if (tt_cell(tt_next, 0, r, c)) {
                    fill_rect(px + c * 10, oy + 16 + r * 10, 9, 9,
                              tt_cols[tt_next]);
                }
            }
        }
        text(px, oy + 70, "PTS", 0xAAAAAA, pack(12, 10, 24), 0);
        gnum(px, oy + 82, tt_score, 0xFFFF00, pack(12, 10, 24));
        text(px, oy + 104, "LIN", 0xAAAAAA, pack(12, 10, 24), 0);
        gnum(px, oy + 116, tt_lines, 0x00FF00, pack(12, 10, 24));
        text(px, oy + 138, "NIV", 0xAAAAAA, pack(12, 10, 24), 0);
        gnum(px, oy + 150, tt_level, 0x40C0FF, pack(12, 10, 24));
    }
    text(cx + 8, cy + ch - 14, "setas move cima gira esp cai (q sai)",
         0xAAAAAA, pack(12, 10, 24), 0);
    if (tt_over) {
        text(ox + TT_W * cell / 2 - 80, oy + TT_H * cell / 2 - 8,
             "FIM DE JOGO (enter)", 0xFF4040, C_BLACK, 0);
    }
}

/* --- Torus 3D (dona achatada sombreada, testa fill_tri + painter) --- */
#define TO_NU 12
#define TO_NV 8
#define TO_R 922   /* 0.9 em Q10 */
#define TO_RR 358  /* 0.35 em Q10 */
#define TO_D 6144
static int torus_ang = 0;

static void draw_torus_client(int cx, int cy, int cw, int ch)
{
    int iu, iv, q, a, b;
    /* rotacionados + projetados; ultima coluna/linha repetem a 1a */
    int sx[TO_NU + 1][TO_NV + 1], sy[TO_NU + 1][TO_NV + 1];
    int32_t rz[TO_NU + 1][TO_NV + 1];
    int qz[TO_NU * TO_NV], order[TO_NU * TO_NV];
    int sa = fsin(torus_ang), ca = fcos(torus_ang);
    int st = fsin(21), ct = fcos(21);
    int midx = cx + cw / 2, midy = cy + ch / 2;
    int m = cw < ch ? cw : ch;
    int half = (m / 2 - 12) * 3 / 4;
    int lx = 256, ly = 358, lz = 922;
    sin_build();
    fill_rect(cx, cy, cw, ch, pack(6, 6, 14));
    for (iu = 0; iu <= TO_NU; iu++) {
        int th = iu * 256 / TO_NU;
        int cth = fcos(th), sth = fsin(th);
        for (iv = 0; iv <= TO_NV; iv++) {
            int ph = iv * 256 / TO_NV;
            int cph = fcos(ph), sph = fsin(ph);
            int32_t ring = TO_R + (int32_t)TO_RR * cph / 1024;
            int32_t x = ring * cth / 1024;
            int32_t y = (int32_t)TO_RR * sph / 1024;
            int32_t z = ring * sth / 1024;
            int32_t x1 = (x * ca + z * sa) / 1024;
            int32_t z1 = (-x * sa + z * ca) / 1024;
            int32_t y1 = (y * ct - z1 * st) / 1024;
            int32_t z2 = (y * st + z1 * ct) / 1024;
            int32_t den = TO_D + z2;
            rz[iu][iv] = z2;
            sx[iu][iv] = midx + (int)(x1 * TO_D / den * half / 1024);
            sy[iu][iv] = midy - (int)(y1 * TO_D / den * half / 1024);
        }
    }
    q = 0;
    for (iu = 0; iu < TO_NU; iu++) {
        for (iv = 0; iv < TO_NV; iv++) {
            qz[q] = (int)((rz[iu][iv] + rz[iu + 1][iv] + rz[iu][iv + 1] +
                           rz[iu + 1][iv + 1]) / 4);
            order[q] = q;
            q++;
        }
    ...
    for (a = 0; a < q; a++) {
        int qq = order[a];
        int u = qq / TO_NV, v = qq % TO_NV;
        int th = u * 256 / TO_NU + 128 / TO_NU;
        int ph = v * 256 / TO_NV + 128 / TO_NV;
        int32_t nx = (int32_t)fcos(ph) * fcos(th) / 1024;
        int32_t ny = fsin(ph);
        int32_t nz = (int32_t)fcos(ph) * fsin(th) / 1024;
        int32_t nx1 = (nx * ca + nz * sa) / 1024;
        int32_t nz1 = (-nx * sa + nz * ca) / 1024;
        int32_t ny1 = (ny * ct - nz1 * st) / 1024;
        int32_t nz2 = (ny * st + nz1 * ct) / 1024;
        int32_t dot = (nx1 * lx + ny1 * ly + nz2 * lz) / 1024;
        int bri;
        uint32_t col;
        if (dot < 0) {
            dot = 0;
        }
        bri = 80 + (int)(dot * 175 / 1024);
        if (bri > 255) {
            bri = 255;
        }
        col = pack((uint8_t)(60 * bri / 255), (uint8_t)(190 * bri / 255),
                   (uint8_t)(230 * bri / 255));
        fill_tri(sx[u][v], sy[u][v], sx[u + 1][v], sy[u + 1][v],
                 sx[u + 1][v + 1], sy[u + 1][v + 1], col);
        fill_tri(sx[u][v], sy[u][v], sx[u + 1][v + 1], sy[u + 1][v + 1],
                 sx[u][v + 1], sy[u][v + 1], col);
    }
    text(cx + 8, cy + 6, "torus: 96 quads, painter+flat", 0x88AAFF,
         pack(6, 6, 14), 0);
    torus_ang = (torus_ang + 3) & 255;
}

/* --- Terreno 3D (heightfield em wireframe, testa draw_line) --- */
#define TE_N 14
static int terr_t = 0;

static void draw_terr_client(int cx, int cy, int cw, int ch)
{
    int i, j;
    int sx[TE_N][TE_N], sy[TE_N][TE_N];
    int hh[TE_N][TE_N];
    int sa = fsin(terr_t / 2), ca = fcos(terr_t / 2);
    int midx = cx + cw / 2, midy = cy + ch / 2 + 20;
    int m = cw < ch ? cw : ch;
    int half = (m / 2 - 16);
    sin_build();
    fill_rect(cx, cy, cw, ch, pack(4, 6, 16));
    for (i = 0; i < TE_N; i++) {
        for (j = 0; j < TE_N; j++) {
            int32_t x = (i - TE_N / 2) * 140;
            int32_t z = (j - TE_N / 2) * 140;
            int32_t h = (int32_t)fsin((i * 37 + terr_t) & 255) * 180 / 1024 +
                        (int32_t)fcos((j * 53 + terr_t / 2) & 255) * 120 / 1024;
            int32_t x1 = (x * ca + z * sa) / 1024;
            int32_t z1 = (-x * sa + z * ca) / 1024;
            int32_t den = 4200 + z1;
            int32_t pxx, ph;
            ...
            ph = h * 4200 / den;
            hh[i][j] = (int)h;
            sx[i][j] = midx + (int)(pxx * half / 1024);
            sy[i][j] = midy - (int)(ph * half / 1024) - half / 3;
        }
    }
    for (i = 0; i < TE_N; i++) {
        for (j = 0; j < TE_N; j++) {
            ...
            if (i + 1 < TE_N) {
                draw_line(sx[i][j], sy[i][j], sx[i + 1][j], sy[i + 1][j], col);
            }
            if (j + 1 < TE_N) {
                draw_line(sx[i][j], sy[i][j], sx[i][j + 1], sy[i][j + 1], col);
            }
        }
    }
    text(cx + 8, cy + 6, "terreno: 14x14 wireframe (196 nos)", 0x88FF88,
         pack(4, 6, 16), 0);
    terr_t = (terr_t + 2) & 1023;
}

/* --- Tunel 3D (aneis em perspectiva voando, testa fill-rate) --- */
#define TU_NR 12
static int tun_t = 0;

static void draw_tun_client(int cx, int cy, int cw, int ch)
{
    int i, k;
    int midx = cx + cw / 2, midy = cy + ch / 2;
    int m = cw < ch ? cw : ch;
    int prevx[4], prevy[4], first = 1;
    int32_t ph, prevslot = -100;
    sin_build();
    fill_rect(cx, cy, cw, ch, pack(0, 0, 0));
    ph = tun_t % (TU_NR * 1024); /* 0..12287: ciclo completo */
    for (i = TU_NR - 1; i >= 0; i--) {
        /* slot 0..11 andando p/ perto (recicla); dist 12..1 */
        int32_t slot = (i * 1024 + ph) % (TU_NR * 1024) / 1024;
        int32_t dist = TU_NR - slot;
        int32_t s = (m / 2 - 8) * 4 / dist;
        int ox = fsin((tun_t / 4 + i * 30) & 255) * (m / 16) / 1024;
        int oy = fcos((tun_t / 5 + i * 44) & 255) * (m / 16) / 1024;
        int px[4], py[4];
        int bri = 255 - (12 - dist) * 20;
        uint32_t col;
        if (bri < 30) {
            bri = 30;
        }
        col = pack((uint8_t)(255 * bri / 255), (uint8_t)(120 * bri / 255),
                   (uint8_t)(20 * bri / 255));
        px[0] = midx + ox - s;
        py[0] = midy + oy - s;
        px[1] = midx + ox + s;
        ...
        py[3] = midy + oy + s;
        for (k = 0; k < 4; k++) {
            draw_line(px[k], py[k], px[(k + 1) & 3], py[(k + 1) & 3], col);
            /* conectores: pula quando um anel reciclou (salto) */
            if (!first && slot - prevslot >= -2 && slot - prevslot <= 2) {
                draw_line(prevx[k], prevy[k], px[k], py[k],
                          pack((uint8_t)(120 * bri / 255), 0,
                               (uint8_t)(120 * bri / 255)));
            }
        }
        for (k = 0; k < 4; k++) {
    ...
    /* pontos centrais (poeira voando) */
    for (k = 0; k < 24; k++) {
        int32_t z = ((k * 521 + tun_t * 3) % 3072) + 256;
        int32_t s = (m / 2) * 1024 / z;
        int ox = (int)((k * 791 % 1200) - 600) * s / 1024;
        int oy = (int)((k * 377 % 1200) - 600) * s / 1024;
        px(midx + ox, midy + oy, pack(200, 200, 220));
    }
    text(cx + 8, cy + 6, "tunel: 12 aneis em perspectiva", 0xFFAA55,
         C_BLACK, 0);
    tun_t = (tun_t + 120) & 16383;
}

/* despacha o loop modal p/ o jogo focado */
void game_run_loop(void)
{
    ...
    if (wins[i].type == W_FOON) {
        foon_run_loop();
    } else if (wins[i].type == W_PONG) {
        pong_run_loop();
    } else if (wins[i].type == W_BRK) {
        brk_run_loop();
    } else if (wins[i].type == W_TET) {
        tet_run_loop();
    }
}

void gui_foon_open(void)
{
    if (win_open(W_FOON) >= 0) {
        foon_reset();
        drv_ready("foon", 1);
        app_open_log(W_FOON, "Foon");
    }
}

void gui_pong_open(void)
{
    if (win_open(W_PONG) >= 0) {
        pong_reset();
        drv_ready("arcade", 1);
        app_open_log(W_PONG, "Pong");
    }
}

void gui_brk_open(void)
{
    if (win_open(W_BRK) >= 0) {
        brk_reset();
        drv_ready("arcade", 1);
        app_open_log(W_BRK, "Breakout");
    }
}

void gui_tet_open(void)
{
    if (win_open(W_TET) >= 0) {
        g_seed = (uint32_t)pit_get_ticks() * 1103515245u + 12345u;
        tt_reset();
        drv_ready("arcade", 1);
        app_open_log(W_TET, "Tetris");
    }
}

void gui_torus_open(void)
{
    if (win_open(W_TORUS) >= 0) {
        drv_ready("gpu3d", 1);
        app_open_log(W_TORUS, "Torus");
    }
}

void gui_terr_open(void)
{
    if (win_open(W_TERR) >= 0) {
        drv_ready("gpu3d", 1);
        app_open_log(W_TERR, "Terreno");
    }
}

void gui_tun_open(void)
{
    if (win_open(W_TUN) >= 0) {
        drv_ready("gpu3d", 1);
        app_open_log(W_TUN, "Tunel");
    }
}
...
static int expl_has_preview(void)
{
    if (expl_sel < 0 || expl_sel >= expl_n || expl_isdir[expl_sel]) {
        return 0;
    }
    if (has_txt_ext(expl_names[expl_sel])) {
        return 1;
    }
    {
        size_t n = strlen(expl_names[expl_sel]);
        if (n >= 4) {
            const char* e = expl_names[expl_sel] + n - 4;
            if (e[0] == '.' && e[1] == 'b' && e[2] == 'm' && e[3] == 'p') {
                return 2;
            }
        }
    }
...
static void draw_expl_client(int idx, int cx, int cy, int cw, int ch)
{
    ...
    {
        static const char* btns[4] = { "Acima", "Novo", "Pasta", "Apagar" };
        int b;
        fill_rect(cx, cy + 17, cw, 15, C_TASK);
        for (b = 0; b < 4; b++) {
            int bx = cx + 4 + b * 60;
            fill_rect(bx, cy + 18, 56, 13, C_WHITE);
            text(bx + 6, cy + 20, btns[b], C_BLACK, C_WHITE, 0);
        }
    }
    ph = expl_has_preview() ? 64 : 0;
    rows = (ch - 34 - 12 - ph) / 12;
    if (rows < 1) {
        rows = 1;
    }
    for (r = 0; r < rows && r < expl_n; r++) {
        int ry = cy + 34 + r * 12;
        char nm[72];
        size_t k = 0;
        char szb[16];
        ...
        if (!expl_isdir[r]) {
            ...
            szb[sn++] = 'B';
            szb[sn] = '\0';
            text(cx + cw - 8 - sn * 8, ry + 2, szb,
                 r == expl_sel ? C_WHITE : C_BLACK,
                 r == expl_sel ? C_TITLE : C_WHITE, 0);
        }
    }
    /* pre-visualizacao do selecionado */
    if (ph) {
        int py = cy + ch - 12 - ph;
        char fp[192];
        static uint8_t pv[2048];
        fill_rect(cx, py, cw, ph, pack(240, 240, 230));
        fill_rect(cx, py, cw, 1, C_SHAD);
        expl_fullpath(expl_sel, fp, sizeof(fp));
        if (expl_has_preview() == 1) {
            int nr = fs_read(fp, pv, sizeof(pv) - 1);
            if (nr > 0) {
                int line = 0, col = 0, px = cx + 4, pyy = py + 3, i = 0;
                char lb[64];
                int ln = 0;
                pv[nr] = '\0';
                while (i < nr && line < 5) {
                    ...
                    if (c == '\n' || ln >= 60) {
                        lb[ln] = '\0';
                        text(px, pyy + line * 12, lb, C_BLACK,
                             pack(240, 240, 230), 0);
                        line++;
                        ln = 0;
                        if (c != '\n') {
                ...
                text(cx + 4, py + 3, "(vazio ou virtual)", 0x666666,
                     pack(240, 240, 230), 0);
            }
        } else {
            /* BMP: dimensoes do cabecalho */
            int nr = fs_read(fp, pv, 64);
            if (nr >= 26 && pv[0] == 'B' && pv[1] == 'M') {
                uint32_t w = (uint32_t)pv[18] | ((uint32_t)pv[19] << 8) |
                             ((uint32_t)pv[20] << 16) | ((uint32_t)pv[21] << 24);
                uint32_t h = (uint32_t)pv[22] | ((uint32_t)pv[23] << 8) |
                             ((uint32_t)pv[24] << 16) | ((uint32_t)pv[25] << 24);
                char ib[48];
                int p = 0;
                const char* s = "BMP ";
                uint32_t vv;
                ...
                ib[p] = '\0';
                text(cx + 4, py + 3, ib, C_BLACK, pack(240, 240, 230), 0);
            } else {
                text(cx + 4, py + 3, "(bmp invalido)", 0x666666,
                     pack(240, 240, 230), 0);
            }
        }
    }
    /* barra de status */
    {
        ...
        while (kk > 0 && n < sizeof(sb) - 1) {
            sb[n++] = tmp[--kk];
        }
        {
            const char* s = " itens ";
            while (*s && n < sizeof(sb) - 1) {
                sb[n++] = *s++;
            }
        }
        v = expl_bytes;
        ...
        {
            size_t k = 0;
            while (expl_msg[k] && n < sizeof(sb) - 1) {
                sb[n++] = expl_msg[k++];
            }
        }
        sb[n] = '\0';
    }
    fill_rect(cx, cy + ch - 12, cw, 12, C_TASK);
    text(cx + 4, cy + ch - 10, sb, C_BLACK, C_TASK, 0);
}
...
static void draw_disk_client(int cx, int cy, int cw, int ch)
{
    ...
    {
        char b[32];
        extern uint32_t fs_rom_nfiles(void);
        extern int fs_rom_is_initramfs(void);
        text(cx + 8, y, fs_rom_is_initramfs() ? "initramfs arquivos:" : "ramfs arquivos:", C_BLACK, C_WHITE, 0);
        u64str(fs_rom_nfiles(), b);
        text(cx + 200, y, b, C_BLACK, C_WHITE, 0);
        y += 14;
        uint32_t nf, nb;
        fs_stats(&nf, &nb);
...
static void draw_window(int idx)
{
    ...
    } else if (wn->type == W_FOON) {
        draw_foon_client(cx, cy, cw, ch);
    } else if (wn->type == W_PONG) {
        draw_pong_client(cx, cy, cw, ch);
    } else if (wn->type == W_BRK) {
        draw_brk_client(cx, cy, cw, ch);
    } else if (wn->type == W_TET) {
        draw_tet_client(cx, cy, cw, ch);
    } else if (wn->type == W_TORUS) {
        draw_torus_client(cx, cy, cw, ch);
    } else if (wn->type == W_TERR) {
        draw_terr_client(cx, cy, cw, ch);
    } else if (wn->type == W_TUN) {
        draw_tun_client(cx, cy, cw, ch);
...
{
    *mw = 2 * MENU_COLW + 4;
    *mh = menu_rows() * MENU_IH + 10;
    *mx = 4;
    *my = MENU_H + 2;
}

/* indice -> (coluna, linha) na grade de 2 colunas */
static void menu_pos(int i, int* c, int* r)
{
    int rows = menu_rows();
    *c = i / rows;
    *r = i % rows;
...
static void draw_menu(void)
{
    ...
    for (int i = 0; i < NMENU; i++) {
        ...
        int bx, iy, hov;
        menu_pos(i, &c, &r);
        bx = mx + 32 + c * MENU_COLW;
        iy = my + 5 + r * MENU_IH;
        hov = (mouse_x >= bx && mouse_x < bx + MENU_COLW - 4 &&
               mouse_y >= iy && mouse_y < iy + MENU_IH);
        if (hov) {
            fill_rect(bx, iy, MENU_COLW - 4, MENU_IH, C_TITLE);
        }
        fill_rect(bx + 6, iy + 5, 16, 16, menu_items[i].color);
        text(bx + 28, iy + 9, menu_items[i].label, hov ? C_WHITE : C_BLACK,
...
static int expl_row_at(int idx, int mx, int my)
{
    ...
    int cw = wins[idx].w - 8;
    int ch = wins[idx].h - (3 + TITLE_H + 2) - 4;
    int ph, rows;
    (void)cw;
    if (mx < cx || mx >= cx + cw) {
        return -2;
    }
    /* barra de ferramentas: Acima Novo Pasta Apagar */
    if (my >= cy + 17 && my < cy + 32) {
        if (mx >= cx + 4 && mx < cx + 60) {
            return -3;
        }
        if (mx >= cx + 64 && mx < cx + 120) {
            return -4;
        }
        if (mx >= cx + 124 && mx < cx + 180) {
            return -5;
        }
        if (mx >= cx + 184 && mx < cx + 240) {
            return -6;
        }
        return -1;
    }
...
static void gui_mouse(void)
{
    ...
    /* menu aberto: itens primeiro (grade de 2 colunas) */
    if (start_open) {
        int mx0, my0, mw0, mh0;
        menu_geom(&mx0, &my0, &mw0, &mh0);
        if (mx >= mx0 + 32 && mx < mx0 + mw0 - 4 && my >= my0 + 5 && my < my0 + mh0 - 5) {
            int rows = menu_rows();
            int cc = (mx - (mx0 + 32)) / MENU_COLW;
            int rr = (my - (my0 + 5)) / MENU_IH;
            int item = (cc < 0 || cc > 1 || rr < 0 || rr >= rows) ? -1 : cc * rows + rr;
    ...
    /* janelas */
    {
        ...
        if (hit >= 0) {
            ...
            /* explorer: linhas e botoes da barra */
                ...
                if (r == -3) {
                    ...
                    if (dbl_click(3000 + r)) {
                        if (expl_isdir[r]) {
                            ...
                            char fp[192];
                            expl_fullpath(r, fp, sizeof(fp));
                            if (has_txt_ext(expl_names[r])) {
                                gui_note_open(fp);
                            } else {
                                size_t fn = strlen(fp);
                                if (fn >= 4 && fp[fn - 4] == '.' &&
...
void gui_poll(void)
{
    ...
    /* 3ddd aberto e visivel: anima todo frame (so o ret da janela) */
    {
        ...
        /* demos 3D: mesmo esquema (testam a GPU sozinhas) */
        {
            wtype_t ds[3] = { W_TORUS, W_TERR, W_TUN };
            int k;
            for (k = 0; k < 3; k++) {
                int di = win_find(ds[k]);
                if (di >= 0 && !wins[di].minimized) {
                    gui_invalidate(wins[di].x - 8, wins[di].y - 8,
                                   wins[di].x + wins[di].w + 8,
                                   wins[di].y + wins[di].h + 8);
                }
            }
        }
    }
