...
No terminal gráfico (`isn_terminal`), `help` lista os 78 comandos.
...
Jogos (menu + shell): `foon` (raycaster estilo Doom: WASD, Esp atira),
`pong`, `breakout`, `tetris`. Demos 3D p/ GPU: `torus`, `terreno`,
`tunel`. Detalhes em `DOC_JOGOS.md`.

Rede cabeada: `e1000` + ARP/IPv4/ICMP; `netcfg` mostra o IP,
`ping 10.0.2.2` testa o gateway QEMU. Áudio: PC Speaker com mixer
(`vol 0-100`, `mute`, `audioinfo`; AC97/HDA detectados, DMA no roadmap).
USB: UHCI/OHCI/EHCI/XHCI por polling (`usb` lista portas/velocidades,
`usbmount` cria `/mnt/usb`; BOT completo no roadmap). Vídeo: brilho
por software (`brilho 10-100`). Detalhes em `DOC_USB_AUDIO_BRILHO.md`.
...
usb.c       USB UHCI/OHCI/EHCI/XHCI por polling + stub MSC (/mnt/usb)
audio.c     mixer vol/mute (pulsing no pcspk) + detect AC97/HDA
brightness.c brilho 10-100 por software (hook no present da gui.c)
...
fs.c        ramfs ROM + overlay gravavel (cd/rm -rf/touch/mkdir)
shell.c     built: 78 applets, readline moderno, PATH /bin:/usr/bin
...
gui.c       compositor: WM, apps, terminal, double buffer, dirty-rects
            + jogos (foon/pong/breakout/tetris) + demos 3D (torus/terreno/tunel)
